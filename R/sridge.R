sridge <- function(x, y, cualcv.S = 5, nkeep = 5, numlam.S = 30, niter.S = 50, normin = 0,
    denormout = 0, alone = 0, ncores = Inf) {
    ...
    fin <- rr_se(Xnor, ynor, lamin, deltaesc = delmin, cc_scale = 1, nkeep, niter.S,
        epsilon = 1e-04)
    beta <- fin$coef
...
CVSE <- function(X, y, nfold, lam, gradlib, nkeep, niter.S) {
    # Performs nfold-CV for S-Ridge INPUT beta.ini, scale.ini: initial estimates of
    # regression and scale X,y: data lam: penalization parameter gradlib: degrees of
    # freedom, used to calculate the delta for the M-scale nkeep: number of
    # candidates for full iterations of IWLS niter.S : number of maximum iterations
    # of IWLS OUTPUT mse: resulting MSE (estimated using a tau-scale)
    ...
    for (kk in 1:nfold) {
        ...
        ytest <- y[testk]
        yesti <- y[estik]
        se <- try(rr_se(Xesti, yesti, lamcv, deltaesc, cc_scale = 1, nkeep, niter.S,
            epsilon = 1e-04))
...
const_marina <- function(delta) {
    integrand <- function(x, c) {
        dnorm(x) * rho_marina(x, c)
    }
    expe <- function(c, delta) {
        integrate(integrand, lower = -Inf, upper = Inf, c)$value - delta
    }
    init <- c(0.1, 100)
