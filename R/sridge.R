...
sridge2 <- function(x,  y,  cualcv.S = 5,  nkeep = 5,  numlam.S = 30,  niter.S = 50,  normin = 0,
    ...
    resmaker <- function( Xnor,  ynor,  nkeep,  niter.S,  Beig){ ##  ,  mux, adaptive){
        force(Xnor); force(ynor); force(nkeep); force(niter.S); force(Beig); force(adaptive) ##  ; force(mux)
        function(lamda, delta){
            fin <- rr_se(X = Xnor,  y = ynor,  lambda2 = lamda,
                         deltaesc = delta,  cc_scale = 1,  nkeep,  niter.S,  epsilon = 1e-04)
            beta <- fin$coef
            betaslo <- beta[2:(length(beta))]
            bint <- beta[1]
            res <- ynor - Xnor %*% betaslo - as.vector(bint)
